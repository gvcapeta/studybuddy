		...
		<section>
			<div class="container-fluid collab-content">
				...
				<div class="con-1 left-card">
					<div class="search">
						<form action="search_question.php" method="post" id="search_form" style="display: inline-flex;width: 100%;">
					...
					if (isset($_GET['q'])) {
						$q = $_GET['q'];
						$sql_questions = "SELECT student_questions.*, students.firstname, students.lastname, students.id AS s_id FROM `student_questions` INNER JOIN `students` ON `student_questions`.`student_id` = `students`.`id` WHERE `student_questions`.`tag` LIKE '".$q."' ORDER BY `student_questions`.`id` DESC";
					} else {
									...
									<div class="p-1">
										<div class="bg-white p-4 d-flex justify-content-between">
											<div>
												...
												<h4 class="mb-0">
													<!-- EVAN:
														Reformated the if-else, mas sanay ako sa {} hindi sa : hahaha
														Anyway, in this code the system will check if the file is image.
														if image, then it will show an <img> tag.
														else, it will show the button with filename that will open the file to a new window.
													-->
													<?php
													if ( /*get_field('image')*/$question['image'] != null) {
														if (in_array(end(explode('.', $question['image'])), array('jpg', 'jpeg', 'png', 'gif'))) {
															echo <<<HTML
																<img src="uploads/{$question['image']}" width="350" height="300" alt=""> <br>
															HTML;
														} else {
															echo <<<HTML
															<div><p>
																<button onclick="window.open('uploads/{$question['image']}','_blank')">Open File</button> 
															HTML . $question['image'] . "</p></div><br/>";
														}
													} ?>
													...
													/*$img = mysqli_query($conn, "SELECT image FROM student_questions");
													...
													$url = stristr($question['question'], 'http://') ?: stristr($question['question'], 'https://');
													//echo $url;exit;
													if ($url != '') {
				...
				<form action="add_question.php" method="post" enctype="multipart/form-data">
					<div class="modal-content">
						...
						<div class="modal-body">
							<textarea name="question" id="question" rows="5" placeholder="Explain the details of your problem" required></textarea>
							<label for="post_file">Attach file </label>
							<font color='brown'> &nbsp;&nbsp; </font>
							<input style="padding-top: 10px;" type="file" name="image">
							<h4>Tags</h4>
							<input type="text" name="tags" value="" data-role="tagsinput" class="form-control" required />
							<!--  Added Checkbox for Anonymous -->
							<input type="checkbox" name="isAnonymous" id="anonymous">
